...
const Login = () => {
  const [username, setUsername] = useState('');
    ...
    const handleLogin = async () => {
        try {
            const response = await axios.post('/api/cashier/login', {
                username,
                password,
            });
            console.log(response.data);
            sessionStorage.setItem('loggedInCashier', JSON.stringify(response.user));
    ...
    return (
        <Card style={{ width: '24rem', margin: 'auto', marginTop: '50px' }} className="shadow">
            <Card.Body className="d-flex flex-column align-items-center">
                <Card.Title>Cashier Login</Card.Title>
